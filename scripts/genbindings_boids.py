...
def main():
    ...
    class_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'frameworks', 'runtime-src', 'Classes'))
    ...
    config.set('DEFAULT', 'classdir', class_root)
    ...
    try:
        ...
        args = ('boids', 'lua_boids_auto')
        config = os.path.join(os.path.dirname(__file__), 'boids.ini')
        command = '%s %s %s -s %s -t %s -o %s -n %s' % (python_bin, generator_py, config, args[0], target, output_dir, args[1])
        _run_cmd(command)
