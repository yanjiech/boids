...
#define FUZZY_THRESHOLD (1e-2f)
	//之所以有一次改大了，是有一次应该算是平行的两条线，来回算了几下叉积结果-0.008还是-0.0008了

	inline static int side(float v) //fuzzyEqual返回0，fuzzyGreater返回1，否则返回-1
	{
		if (_equal(v, 0.0f))
		{
			return 0;
	...
	{
		return fabs(x - y) <= FUZZY_THRESHOLD; //mac上面直接用abs会转成int，太坑了。。
	}
...
public:
	...
	static bool clockwiseCloser(cocos2d::Point origin, cocos2d::Point base, cocos2d::Point x, cocos2d::Point y)
	{
		...
		//先看是不是同侧，在(origin, base)右侧的一定要比在左侧的顺时针更近
		int side_x = Fuzzy::side(base_vec.cross(vec_x));
		int side_y = Fuzzy::side(base_vec.cross(vec_y));

		//其实下面这两种特殊情况在我使用这个方法的地方照理是不会进的。。
		if (side_y == 0 && base_vec.dot(vec_y) > 0) //如果vec_y和base_vec完全同向，那x肯定不可能更近了
		{
			return false;
		}

		if (side_x == 0 && base_vec.dot(vec_x) > 0) //如果vec_y没有同向而vec_x同向了，那肯定是x最近了
		{
			return true;
		}
